...
**Status update (2026-09-24): the full-range question above is now answered for two of the four bursts, and the answer is no.** `full_range_all_bursts_check.py` was split into `full_range_single_burst.py` (shared fit/plot logic) plus one driver per burst, `full_range_check_GRB*.py`, so each could be run and monitored independently instead of one script blocking on all four. GRB080916C and GRB140206B both ran to completion:

- **GRB080916C**: `t_peak` stable across all 6 pulses (≤0.3%), but `t_v` exceeds the user's ≤5% tolerance on 4 of 6 pulses (up to ~17%). **Fails.**
- **GRB140206B**: pulses 1/2/3/7 fine; the 23–28s pulse cluster (4/5/6) fails badly — pulse 6's `t_peak` shifts 37%, `t_v` shifts 86%, even with the pulse-6 reseed already diagnosed in `experiments/normalized_vs_unnormalized_fit/comparison.md`. **Fails.**
- GRB131014A and GRB231129C were killed mid-run (not completed) once the session's focus moved to the per-burst treatment below.

**Net conclusion so far: switching `lorentz_factor.py`'s `Gamma_min` input to the full range cannot be a single project-wide decision** — two of four bursts already fail the stated tolerance outright, so this will need a burst-by-burst (plausibly pulse-by-pulse) call once all four have a clean result, not a blanket switch.

**Separately, same session:** each burst also got its own dedicated folder — `codes-for-paper/variability_analysis/GRB080916C/`, `GRB131014215/`, `GRB140206275/`, `GRB231129C/` — each with `_common.py` + `fitter_normalized.py` + `fitter_unnormalized.py`, promoting the normalized-vs-unnormalized comparison (previously only a point-estimate check in `fit_comparison.py`) to the full production treatment: episode matching, each burst's own already-established LAT-photon-assignment rule, and MC-propagated `t_v` for *both* methods (the unnormalized `least_squares` fit's covariance is derived from its Jacobian the same way `scipy.optimize.curve_fit` does internally, so both methods' uncertainties are the same kind of estimator). GRB080916C also got a standalone top-level `fitter_GRB080916C.py` (self-contained, no `_common.py` dependency, `_fullrange`-suffixed outputs so it never collides with `fitter.py`'s production `(-1, 70)`-window results) — the user has since been iterating its `P0` directly, so treat its on-disk value as live, not settled.

Results: GRB080916C and GRB131014A both ran clean on both methods and agree well pulse-for-pulse (GRB131014A's pulses 1/2 disagreeing between methods reproduces this section's own already-documented "unresolved" finding for that burst — not a new problem). GRB140206B surfaced a real, apparently genuine 21% `t_v` disagreement on pulse 5/TR3 between methods (after correcting for an unrelated pulse-index label swap on pulses 2/7 that initially looked like an 800%+ discrepancy but wasn't) — not yet diagnosed. GRB231129C's folder was built but not run: mid-session the user found that burst's light curve/detector set had changed significantly (more NaI detectors than previously accounted for), and is now handling its refit manually, directly in production `fitter_GRB231129779.py`, rather than through the new folder track.

**The user is continuing this work on a heavier machine.** Full detail, including the exact per-pulse comparison tables: `variability_analysis.md`'s 2026-09-24 session-update section (top-level `codes-for-paper/variability_analysis/` folder). Also written this session, purely speculative and not scheduled: `t_peak_tv_reparametrization_idea.md`, sketching a possible `(t_s,tau1,tau2) -> (t_peak,t_v,r)` reparametrization of the Norris pulse, meant to be shown to other models/reviewers for a second opinion before any decision on whether it's worth a real trial.

**This entire 2026-09-24 batch is exploratory/testing work, and the user intends to stage it for a git commit** — an explicit exception to this project's working-tree-only rule, same precedent as the RNG/seeding overhaul commits and the 2026-09-23 Phase 5/BUG-23 commit (see `HANDOFF.md` §13 and the 2026-09-23 paragraph above). None of it has been fed into `lorentz_factor.py`'s `Gamma_min` pipeline, and two of four bursts already failed the ≤5% tolerance check above, so a commit here checkpoints testing work in progress, not a finished or paper-ready result.
