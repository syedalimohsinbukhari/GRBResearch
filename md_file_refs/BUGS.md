...
**Superseding fix, 2026-09-23 (user decision): sum all NaI detectors instead of picking one.** The "hardcode a detector per burst" fix proposed above was not applied. Instead, since every `fitter*.py`/`window_sensitivity.py` already had a separate, longer-standing open issue (flagged 2026-09-18 in `variability_analysis.md`, "every fit uses only a single NaI detector rather than summing across detectors, unlike typical published GRB analyses"), the user chose to resolve both issues at once: every script now sums **all** of a burst's NaI detectors' background-subtracted count rates (raw, **no per-detector normalization** — each detector's own effective area/viewing angle is trusted to weight its own contribution, the user's explicit choice over peak-normalizing or integral-normalizing each detector first, or an angle/effective-area-weighted combination). This structurally closes BUG-23 rather than patching around it: there is no longer a single detector to mis-pick, so `os.listdir()` order can never matter again, for any burst.

Applied to all 9 affected files: `fitter.py`, `fitter_CLAUDE_GRB131014215.py`, `fitter_GRB140206275.py`, `fitter_GRB140206275_simple.py`, `fitter_GRB231129779.py`, and the four matching `experiments/window_sensitivity_GRB*/window_sensitivity.py` copies. Each detector's time grid is asserted identical to the first before summing (confirmed identical for all four bursts' detector sets by direct check, not assumed — `n3`/`n4` for GRB080916C, `n9`/`na`/`nb` for GRB131014A, `n0`/`n1`/`n3` for GRB140206B, `n3`/`n6`/`n7` for GRB231129C, all matching to the element on a shared 64 ms grid). All 9 files compile (`py_compile`) after the change.

**All four bursts' `fitter*.py` scripts re-run, 2026-09-23 — all converge, one required a further fix:**
- **GRB231129C**: converged cleanly with the existing 5-pulse seed, no changes. `mc_kept_fraction` improved for every pulse (e.g. 0.35→0.72, 0.94→0.99) — combining detectors genuinely helped S/N here, not diluted it.
- **GRB140206B** (`fitter_GRB140206275.py`'s SIMPLE/COMPLEX pair and `_simple.py`): converged cleanly with the existing seeds, no changes.
- **GRB131014A**: failed to converge with either the original seed or a seed reconverged from the old single-detector fit (`RuntimeError: Optimal parameters not found`). Diagnosed as a pure iteration-budget issue, not a bad seed or a different optimum — both seeds land on the same solution once `max_iterations` is raised from `NorrisFitter`'s default (5000) to 20000 (the summed 3-detector curve's peak, ~130,030 cts/s, is ~3x the single-detector curve's, ~44,084). Fixed by passing `max_iterations=20000` in `fitter_CLAUDE_GRB131014215.py`; same bump applied to `fitter.py` as a precaution.
- **GRB080916C**: converged, but the 7th pulse (`t_s≈20`, inside TR3) landed on a genuinely different, degenerate optimum: `tau2` collapsed to 0.0087 (was 1.02), `mc_kept_fraction` dropped from 0.52 to 0.05 — a visible near-delta-function spike in the plot at t≈20.5s that the raw summed data doesn't support (that region sits at ~1650-1700 cts/s against a ~1000-1800 cts/s noisy floor throughout, no real standout feature). Confirmed not a seed/iteration artifact: both the original seed and one reconverged from the old single-detector fit land on the same degenerate solution at every iteration budget tried up to 50000. **User decision (2026-09-23): drop this pulse, reverting to the 6-pulse model** (`P0_6`, already an independent baseline in `experiments/window_sensitivity_GRB080916009/window_sensitivity.py`) rather than reseed or bound `tau2`. Verified the drop doesn't disturb the other six pulses first: every other pulse's `t_peak`/`t_v`/`A` matches the (still-degenerate) 7-pulse fit's corresponding pulse to within ordinary `t_s`<->`tau1`-degeneracy noise (largest `t_peak` shift 0.03s), and total SSE rises only ~0.4% (7.202e7 vs 7.173e7, physical-units counts/s²) — the signature of dropping a pulse that was fitting noise, not a real feature. Applied to `fitter.py`; regenerated plot confirmed spike-free.
