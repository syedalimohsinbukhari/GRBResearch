...
**Staleness note (added 2026-09-16, commit refs updated 2026-09-23):** this file's narrative sections (§1–§13) stop at the RNG/seeding overhaul (2026-09-03). Both repos have moved on since — `GRBResearchWork` is now at `[main-minor-95]` (was `[main-minor-75]`/the `seed-correction` branch at the time §13 was written; `[main-minor-88]` as of when this note was first added, `[main-minor-91]`, then `[main-minor-93]` as of the previous two updates) and `GRBResearchPaper` is still at `[main-minor-21]` (was `[main-minor-15]`, unchanged since this note was added) — including at least a GBM-only 3-way refit exercise (`codes-for-paper/gbm_only_refit/`) and the Phase 5 variability-timescale work below, neither reconstructed here. Treat `git log` in each repo, `GRBResearchWork/PHASE5_TV_PLAN.md`, and `BUGS.md` (now 23 bugs + 11 observations + 1 plan note — all closed/resolved except OBS-11, DEFERRED; BUG-23 closed 2026-09-23, see below) as more current than this file's specifics; the status table in §1 has been spot-corrected for Phase 5 and the weakness-review pass (§12) only.

Everything below is **working-tree only. Nothing has been committed in either repo**, per the standing rule — **except §13** (RNG/seeding overhaul), where the user explicitly asked for commits partway through that session, and **the 2026-09-23 Phase 5/BUG-23 session** (`GRBResearchWork` commit `[main-minor-95]`), where the user again explicitly asked for a commit; see §13 and the Phase 5 status row below respectively for what each covers.
...
| Optional: Norris-profile $t_v$ (see §6b) | 🟨 In progress since 2026-09-07, last updated 2026-09-23 — see `GRBResearchWork/PHASE5_TV_PLAN.md` and `codes-for-paper/variability_analysis/variability_analysis.md`; not yet fed into `lorentz_factor.py`. GRB080916C reverted to a standard 7-pulse fit 2026-09-22 (was an 8-pulse `norris2-1`/`norris2-2` split, judged unsound). **BUG-23 (wrong/non-deterministic NaI detector selection) closed 2026-09-23** via a structural fix — every `fitter*.py`/`window_sensitivity.py` now sums *all* of a burst's NaI detectors, raw, no per-detector normalization, removing the single-detector pick entirely, for all four bursts (was fixed for GRB080916C only as of 2026-09-22). Re-running all four fits with the fix surfaced two more fixes: GRB131014A needed `max_iterations=20000` (iteration-budget issue), and GRB080916C's 7th pulse turned out degenerate on the summed curve and was dropped (now 6 pulses). A new standalone folder, `codes-for-paper/variability_analysis/experiments/normalized_vs_unnormalized_fit/`, tested whether peak-normalization changes the fit (it doesn't — SSE agrees to <0.15% for all four bursts) and diagnosed (not yet applied) a recommended full-range configuration for GRB140206B's two weakest pulses. **Status update (2026-09-24):** the full-range stability check was split into one script per burst (`full_range_check_GRB*.py`) so each could run independently; GRB080916C and GRB140206B both ran to completion and **failed** the user's ≤5% `t_peak`/`t_v` tolerance (GRB080916C: 4/6 pulses over on `t_v`; GRB140206B: the 23–28s pulse cluster badly over — pulse 6's `t_peak`/`t_v` shift 37%/86%), so a blanket full-range switch for `Gamma_min` is not viable as a single project-wide decision. Separately, each of the four bursts also got its own `GRB*/` folder (`_common.py` + `fitter_normalized.py` + `fitter_unnormalized.py`) promoting the normalized-vs-unnormalized full-range comparison to the full production treatment (episode matching, LAT-photon assignment, MC-propagated `t_v`) rather than `fit_comparison.py`'s lighter check — GRB080916C and GRB131014A ran clean (GRB131014A's pulses 1/2 disagreeing between methods reproduces this file's own already-known "unresolved" finding for that burst, not a new problem); GRB140206B surfaced a real, still-undiagnosed 21% `t_v` disagreement on pulse 5/TR3 after correcting for an unrelated pulse-index label swap between the two methods' fits; GRB231129C's light curve/detector set changed significantly mid-session and its refit is being done manually, not through this track. **The user is continuing this work on a heavier machine.** This entire 2026-09-24 batch (the split scripts, the four new `GRB*/` folders, `fitter_GRB080916C.py`, the `window_sensitivity_GRB140206275` split, `t_peak_tv_reparametrization_idea.md`) is exploratory/testing work that the user intends to **stage for a git commit** — an explicit exception to the standing working-tree-only rule, same precedent as §13 and the 2026-09-23 commit. Not yet in `Gamma_min`, and two of four bursts already failed the ≤5% tolerance, so this is a checkpoint of testing work, not a settled result. Full detail: `PLAN.md`'s Phase 5 section and `codes-for-paper/variability_analysis/variability_analysis.md`'s 2026-09-24 session-update section. |
