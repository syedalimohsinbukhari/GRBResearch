"""GRB131014A, UNNORMALIZED method, fit over the light curve's own full x.min()/x.max() range.
...
scipy.optimize.curve_fit does internally). Same 5-pulse P0, same nearest-preceding-onset LAT-photon
assignment and episode-matching logic as fitter_normalized.py in this folder -- only the fitting method
differs.
"""
...
csv_path = Path(__file__).parent / f"norris_fit_results_{GRB_131014.name}_unnormalized.csv"
...
fig_path = Path(__file__).parent / f"norris_fitted_{GRB_131014.name}_unnormalized"
