"""GRB080916C, UNNORMALIZED method, fit over the light curve's own full x.min()/x.max() range.

Same raw-counts/explicit-x_scale scipy.optimize.least_squares path as
experiments/normalized_vs_unnormalized_fit/fit_comparison.py's Method 2 (pymultifit's BaseFitter.fit()
doesn't expose x_scale/method, so this bypasses NorrisFitter entirely -- see that file's docstring for
why x_scale='jac' was tried and rejected). Same 6-pulse P0, same LAT-photon assignment and
episode-matching logic as fitter_normalized.py in this folder -- only the fitting method differs.

Covariance for MC-propagated t_v: least_squares doesn't return one directly (unlike curve_fit, which
NorrisFitter uses internally). Computed here the same way scipy.optimize.curve_fit derives pcov from a
least_squares result when absolute_sigma=False (its default) -- cov = inv(J^T J) * s_sq, with
s_sq = 2*cost / (n_points - n_params) the reduced chi-square scale factor -- so both methods' t_v
uncertainties come from conceptually the same estimator, not two different ones.
"""
...
# Covariance from the Jacobian at the solution -- see module docstring for the formula, which matches
# scipy.optimize.curve_fit's own internal derivation (absolute_sigma=False, its default).
...
csv_path = Path(__file__).parent / f"norris_fit_results_{GRB_080916C.name}_unnormalized.csv"
...
# NorrisFitter/BaseFitter.plot_fit() here, since this fit never went through pymultifit), same visual
# convention (dashed individuals, one shade per pulse) as fit_comparison.py's plot.
...
fig_path = Path(__file__).parent / f"norris_fitted_{GRB_080916C.name}_unnormalized"
