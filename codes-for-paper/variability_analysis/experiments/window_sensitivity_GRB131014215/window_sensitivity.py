...
# count rates raw, with no per-detector normalization -- matching fitter_CLAUDE_GRB131014215.py's own fix
# and the existing "n3+n4 summed" ROOT cross-check precedent (variability_analysis.md). This structurally
# closes BUG-23: there's no longer a single detector to mis-pick via os.listdir()[0]. Detector time grids
# are confirmed identical before summing, not assumed.
