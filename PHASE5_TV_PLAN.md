...
Work continues instead as a **separate, manually-driven, GRB-by-GRB joint-Norris-fit track** in `codes-for-paper/variability_analysis/` (see that folder's `variability_analysis.md`), started 2026-09-07. All four bursts now have a manual fit as of 2026-09-16/17: GRB131014A (5 pulses), GRB140206B (two candidate models, SIMPLE/COMPLEX), GRB231129C (5 pulses), and GRB080916C (8 pulses, the last to be done — the one where the automated approach originally stalled). None of the four are written up in this session's full detail in `variability_analysis.md`'s main sections yet (only GRB131014A is); GRB080916C's decomposition and a CERN ROOT cross-check of it do have their own dedicated section there. The "New files" section below (§ "New files — `GRBResearchWork/variability_timescale/`") describes the abandoned automated layout and no longer reflects what's on disk — kept here for the historical methodology reasoning (energy band, eq. 10 sourcing, EX0/EX1 window-inheritance rule, etc.), which still applies to the manual track, not for the file list.
...
**Quadrature-error bug, fixed 2026-09-18 (BUG-22, now closed).** `light_curves.py`'s `lightcurve_data(..., errors=True)` combined per-energy-channel errors linearly instead of in quadrature, inflating the combined error by ~9.4x for a typical 10–400 keV band (89 channels summed). Found via a CERN ROOT cross-check of GRB080916C's fit coming back `chi2/ndf ≈ 0.03`; fixed in all six copies of the function across the repo. Confirmed zero impact on anything published — `errors=True` was never actually called anywhere else. See `BUGS.md` BUG-22 and `variability_analysis.md`'s GRB080916C section.

**Open issue, flagged 2026-09-18, not yet addressed: single-detector fits.** Every manual fit in this track (all four bursts) uses only one NaI detector (`nai_data[0]`, whichever sorts first alphabetically) rather than summing across the GRB's full NaI detector set — not a deliberate methodological choice, just what the very first script did and every later one copied. The user's own reading is that published GRB analyses typically do combine detectors. Reworking this — which detectors to include, how to combine rate/background/errors across them (the quadrature-vs-linear distinction from BUG-22 applies here too) — is planned follow-up work; every `t_v`/pulse-decomposition result produced so far predates it.
